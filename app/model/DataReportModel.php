...
class DataReportModel extends MedooOrm
{
    ...
    public static function filterWinLossRecordsForUser($user_id,$filters = [],bool $isForOnlyUser = false,bool $addAgent = true){
       ...
       $query = "SELECT    (SELECT SUM(expenditure_amount) FROM {$table_name} WHERE uid IN (".implode(',',$place_holders).") AND bet_type = 1AND order_type != 11 {$where_clause}) AS total_normal_bet_amount, 
