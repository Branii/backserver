...
class DataReportModel extends MedooOrm
{
   public static function fetch_users_win_loss_report($partnerID, int $currentPage = 1, $limit = 10, string $orderColumn = "uid"): array
   {
      try {
         ...
         $stmt = $db->query("SELECT *,(SELECT COUNT(*) FROM {$table_name}) AS num_users FROM {$table_name} ORDER BY {$orderColumn} DESC LIMIT :offset, :limit", [
            ':offset' => $offset,
            ':limit' => $limit,
         ]);
   ...
   {
      $res = parent::openLink($partnerID)
         ->query(
            "SELECT gt_id,name  FROM game_type WHERE name LIKE :search ORDER BY  CASE 
   ...
   public static function fetchUser($user_id)
   {
      ...
      return parent::getLink()
         ->query("SELECT * FROM users_test WHERE uid = :uid LIMIT 1", [
            ':uid' => $user_id,
         ])
         ->fetch(PDO::FETCH_OBJ);
   }
   ...
   public static function filter_fetch_users_for_win_loss_report($userData, int $currentPage = 1, int $recordsPerPage = 10, string $tblid = "uid")
   {
      try {
         ...
         $results = $service->query("SELECT *,(SELECT COUNT(*) FROM {$table_name}) as num_users FROM {$table_name} WHERE uid= :uid ORDER BY $tblid DESC", ["uid" => is_object($userData) ? $userData->uid : $userData])->fetch(PDO::FETCH_OBJ);
   ...
   public static function filterWinLossRecordsForUser($partnerID, $user_id, $filters = [], bool $isForOnlyUser = false, bool $addAgent = true)
   {
      try {
         ...
         if ($user_and_subs["status"] === "error") {
            return $user_and_subs;
         }
         $place_holders = [];
         $params = [];
         $track_bet_params = [];
         ...
         $query =
            "SELECT    (SELECT SUM(expenditure_amount) FROM {$table_name} WHERE uid IN (" .
            implode(',', $place_holders) .
            ") AND bet_type = 1 AND order_type != 11 {$where_clause}) AS total_normal_bet_amount, 
       (SELECT SUM(expenditure_amount) FROM {$table_name} WHERE uid IN (" .
            implode(',', $place_holders) .
            ") AND bet_type = 1 AND order_type = 12 {$where_clause}) AS total_loss_amount,
       (SELECT SUM(account_change) FROM {$table_name} WHERE uid IN (" .
            implode(',', $place_holders) .
            ") AND bet_type = 1 {$where_clause} AND order_type = 3)  AS total_win_amount,(SELECT SUM(expenditure_amount) FROM {$table_name} WHERE uid IN (" .
            implode(',', $place_holders) .
            ") AND bet_type = 1 AND order_type = 11 {$where_clause}) AS total_direct_refund_amount,(SELECT SUM(expenditure_amount) FROM {$table_name} WHERE uid IN (" .
            implode(',', $place_holders) .
            ") AND (order_type = 2 || order_type = 10)   {$where_clause}) AS total_promotions_and_bonus, 
       (SELECT SUM(account_change) FROM {$table_name} WHERE uid IN(" .
            implode(',', $place_holders) .
            ") AND (order_type = 7 || order_type = 8) {$where_clause}) AS total_rebate_amount,
       ...
       (SELECT COUNT(DISTINCT(uid)) FROM {$table_name} WHERE uid IN(" .
            implode(',', $place_holders) .
            ") AND (order_type = 3 || order_type = 12) {$where_clause}) AS num_bettors,
       (SELECT COUNT(trans_id) FROM {$table_name} WHERE uid IN(" .
            implode(',', $place_holders) .
            ") AND (order_type = 3 || order_type = 12) AND bet_type = 1 {$where_clause}) AS num_bet_tickets 
       FROM {$table_name} WHERE uid IN (" .
         ...
         $query =
            "SELECT SUM(done_amount) as total_done_amount, SUM(win_amount) AS total_win_amount, SUM(lost_amount) AS total_loss_amount,SUM(total_amount) as total_track_amount,SUM(refund_amount) as total_refund_amount, SUM(tracked) as track_num_bets FROM trackbet WHERE user_id IN (" .
            implode(',', $track_place_holder_uids) .
            ") {$track_where_clause}";
         ...
         $win_loss = $isForOnlyUser
            ? ($total_bet_amount === 0
               ? self::formatNumber($total_rebate_amount)
               : self::formatNumber($total_valid_amount + $total_fee_amount - ($total_win_amount + $total_rebate_amount + $total_promotions_and_bonus + $total_refund_amount)))
            : self::formatNumber($total_valid_amount + $total_fee_amount - ($total_win_amount + $total_rebate_amount + $total_promotions_and_bonus + $total_refund_amount));
         $results = [
            "user_id" => $user_id,
            "username" => $username,
            "account_type" => $account_type,
            "num_bettors" => $results->num_bettors,
            "num_bet_tickets" => $results->num_bet_tickets + $track_num_bets,
            "rebate" => $rebate,
            "total_bet_amount" => self::formatNumber($total_bet_amount),
            "total_rebate_amount" => self::formatNumber($total_rebate_amount),
            'total_win_amount' => self::formatNumber($total_win_amount + ($results->win_bonus ?? 0)),
            "total_valid_amount" => self::formatNumber($total_valid_amount),
            'win_loss' => $win_loss,
            "num_subs" => max(count($subs_data) - 1, 0),
            "fees" => self::formatNumber($total_fee_amount),
            "total_promotions_and_bonus" => self::formatNumber($total_promotions_and_bonus),
   ...
   public static function allSubs($paretner,$agent_id, int $currentPage = 1, int $limit = 10, bool $addAgent = false)
   {
      try {
         $table_name = "users_test";
         $db = parent::openLink();
         ...
         $sql = "SELECT *,
            ...
            ORDER BY uid DESC 
            LIMIT :offset, :limit";
         $stmt = $db->query($sql, [':agent_id' => $agent_id, ':offset' => $offset, ':limit' => $limit]);
         // Fetch results
         $data = $stmt->fetchAll(PDO::FETCH_OBJ);
         return self::response($data);
      } catch (Exception $e) {
