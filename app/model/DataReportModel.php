...
class DataReportModel extends MedooOrm {
    ...
    public static function filter_fetch_users_for_win_loss_report($usersData, int $currentPage = 1, int $recordsPerPage = 10, string $tblid = "uid")
    {
        ...
        (SELECT COUNT(*) FROM {$table_name} AS sub WHERE sub.agent_id = {$table_name}.agent_id AND sub.agent_id != '*****') AS agent_sub_count, 
        (SELECT COUNT(*) FROM {$table_name} AS sub WHERE sub.agent_id = {$table_name}.uid) AS sub_count, 
        (SELECT sub.username FROM {$table_name} AS sub WHERE sub.uid = {$table_name}.agent_id AND sub.agent_id != '*****') AS agent_username 
    FROM {$table_name} 
    ...
    public static function getBetTicketsNumForUser($user, $bet_tables = [], $userData): array
    {
        ...
        $relationship           =  $user->account_type == 2 ? "<button class='btn tag' > Top Agent </button>" : "<button class='btn tag user-rel'  data-bs-target='#user-rel-modal' data-bs-toggle='modal'> " .  (empty(trim($user->agent_name)) ? "" : ($agent_sub_count == 1 ? "{$user->agent_name}>$user->username" : "{$user->agent_name}>$user->username...")) . " </button>";
        ...
        $win_loss = ($total_win_amount + $total_rebate_amount) - $total_valid_amount;
