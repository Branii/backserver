...
class PaymentPlatformModel extends MEDOOHelper
{
    ...
    public static function searchPlatformNames($platformName): Mixed
    {
        ...
        $query = parent::openLink()->query("SELECT * FROM currency WHERE name LIKE :search AND currency_type='fiat' LIMIT 50", ['search' => "%$query%"]);
        ...
        try {
            ...
            $query = parent::openLink($db_id)->query("SELECT * FROM currency WHERE name LIKE :search AND currency_type='fiat' LIMIT 50", ['search' => "%$query%"]);
            $data = $query->fetchAll(PDO::FETCH_OBJ);
            return $data;
        } catch (Exception $e) {
            return self::response("Internal Server Error." . $e->getMessage(), false);
    ...
    public static function FetchPaymentPlatform($page, $limit): array
    {
        ...
        $query = "SELECT bankid,name,bank_type,currency_type,timezone,
    ...
    {
        $startpoint = $page * $limit - $limit;
        $query = "SELECT *,(SELECT COUNT(*) FROM payment_platforms) as  FROM payment_platforms  ORDER BY cid DESC LIMIT :offset, :limit 
         ";
        $data = parent::query($query, ['offset' => $startpoint, 'limit' => $limit]);
        return ['status' => "success", 'data' => $data];
    }

    //fetchDifferentCurrency
    //  public static function fetchDifferentCurrency(){
    ...
    public static function searchPaymentPlatform($page, $limit, $searchData = []): Mixed
    {
        try {
            ...
            if (!empty($searchData["startDate"]) || !empty($searchData['endDate'])) {
                if (empty($searchData['endDate'])) {
                    $where_clause[] = "date_created=:datecreated";
                    $params[":datecreated"] = $searchData['startDate']; // Corrected to startDate
                } elseif (empty($searchData['startDate'])) {
                    $where_clause[] = "date_created=:enddate";
                    $params[":enddate"] = $searchData['endDate'];
                } else {
