$(() => {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (userObj.account_type > 1) {
          if (!$(".get-user-details-btn").hasClass("btn-disabled"))
  ...
  $(document).on("click", ".wl-refreshlist", function () {
    ...
    $("#wl-enddate").val("");
    $("#winLossDtholder").html(
      `<tr class="no-resultslist"> <td colspan="13" id="wl-empty-td"><img src="/admin/app/assets/images/not_found.jpg" class="dark-logo" alt="Empty Results" /></td></tr>`
    );
    $("#wl-pagination-wrapper").html("");
    $("#paging_infowl").html("---------");
  ...
  $(document).on("click", ".back-btn", function () {
    ...
    if (historyStack.length === 0) {
      ...
      if (
        $($("#winLossDtholder").find("tr:first-child")[0]).attr(
          "data-acc-type"
        ) == "agent" &&
        $("#winLossDtholder").children().length === 1
      ) {
        if ($(".get-user-details-btn").hasClass("btn-disabled"))
          $(".get-user-details-btn").removeClass("btn-disabled"); // remove disabled btn class from the get-user-details-btn
      }
    }
  });
  ...
  const renderwithdrawPagination = (
    totalPages,
    currentPagewithdraw,
    type = "page-top-agents"
  ) => {
...
const getUserRowMarkup = (userData) => {
  ...
  }' class='wl-results' data-acc-type='${
    userData.account_type > 1 ? "agent" : ""
  }'>
