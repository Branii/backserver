...
// showToast(translations.headsUp, translations.selectDataFields, "info");
      ...
      html += `
                      <tr class="trow">
                        ...
                        <td><i class='bx bxs-circle' style='color:${
                          statusColor[item.order_type].color
                        };font-size:8px;margin-right:5px;'></i>${
        statusColor[item.order_type].title
      }</td>
                        <td>${
        ...
        item.order_type
      }' class='bx bx-info-circle tinfo' style='color:#868c87;font-size:18px;cursor:pointer;'></i></td>
                      </tr>
                  `;
    });
 ...
 showToast(
  ...
  async function filterTrasaction(
    ...
    try {
      const response = await fetch(
        `../admin/filtertransactions/${transusername}/${transactionId}/${ordertypetrans}/${partneruid}/${startdatrans}/${enddatetrans}/${currentPage}/${pageLimit}`
      );
  ...
  async function fetchTrasactionBet(transactionId) {
    try {
      const response = await fetch(
        `../admin/getTransactionBet/${transactionId}`
      );
