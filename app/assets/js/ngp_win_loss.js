$(() => {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.length == 0) {
          historyStack = [];
          $("#win-loss-dtholder").html(
            "<tr><td colspan='12'  style='text-align:center;'>No results found.</td></tr>"
          );
          return;
        }
        if ($("#user-details-btn").is(":visible"))
          $("#user-details-btn").hide();
        if (num_res === 1) {
  ...
  $(document).on("click", ".ngp-fetch-user-details", function () {
    ...
    if (numTableRows.length > 1) {
      showToast(
        "Too many users available.",
        "Please select a single user.",
        "info"
      );
      return;
    }

    const userID = $("#ngp-winLossDtholder")
      .find("tr:first-child")
      .attr("id")
      .split("-")[2];
  ...
  $(document).on("click", ".ngp-wl-refreshlist", function () {
    ...
    $("#ngp-wl-enddate").val("");
    $("#ngp-winLossDtholder").html(
      `<tr class="no-resultslist"> <td colspan="13" id="ngp-wl-empty-td"><img src="/admin/app/assets/images/not_found.jpg" class="dark-logo" alt="Empty Results" /></td></tr>`
    );
    $("#ngp-wl-pagination-wrapper").html("");
    $("#ngp-paging_infowl").html("---------");
