$(function () {
    ...
    const usernotiData = (data) => {
        ...
        data.forEach((item) => {
            const readstatus =
                item.read_status === "read" ? `<span class="badge fw-semibold py-1 w-85 bg-success-subtle text-success">${transRead}</span>` : `<span class="badge fw-semibold py-1 w-85 bg-warning-subtle text-warning">${transUnread}</span>`;
    ...
    async function fetchusernotification(page, pageLimit) {
        try {
            const response = await fetch(`../announce/fetchusernotification/${page}/${pageLimit}`);
            ...
            document.getElementById("paging_infonoti").innerHTML = `${txtPage} ${page} ${txtOf} ${data.totalPages} ${txtPages}`;
        } catch (error) {
    ...
    async function filterUserNotifys(username, messagestype, startnotfys, endnotfys, currentPage, pageLimit) {
        $.post(`../announce/filteruserNotifys/${username}/${messagestype}/${startnotfys}/${endnotfys}/${currentPage}/${pageLimit}`, function (response) {
            try {
