$(function() {
  ...
  function tableScrollWithdraw() {
    const tableContainerFinanceWidrl = document.querySelector(
  ...
  async function fetchwithdraw(pagewithdraw) {
    try {
      let response = await fetch(
        `../financial/fetchwithdraw/${partnerID}/${pagewithdraw}/${pageLimit}`
      );
      response = await response.json();
      if (response.status === "error") {
        $("#withdrawContainer").html(
          `<tr class="no-resultslist"><td colspan="15">Error: ${response.data}</td></tr>`
        );
        return;
      }
  ...
  function renderwithdrawPagination(totalPages, currentPagewithdraw) {
    ...
    pagLink += `
      <li class='page-item ${currentPagewithdraw === 1 ? "disabled" : ""}'>
        <a class='page-link' href='#' data-page='${currentPagewithdraw -
    ...
    $("#paging_infowithdraw").text(
      `${langStrings.Page} ${currentPagewithdraw} ${langStrings.of} ${totalPages} ${langStrings.pages}`
    );
  }
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        const totalPages = Math.ceil(
          withdrawalRecords[0].total_records / limit
        );
    ...
    $.post(`../admin/Searchusername/${encodeURIComponent(query)}`, function(
      ...
      try {
        ...
        for (let index = 0; index < response.length; index++) {
          ...
          optionsHtml +=
            username === undefined
              ? `<li class="optionlist"> No Data Found.</li>`
              : `<option class="optionlist" value="${user.uid}" data-username="${username}">${username}</option>`;
        }
