$(() => {
    ...
    $(document).on("click", ".gameon,.gameoff", function () {
        ...
        $("#toggle-lottery-msg").html(
            `Are you sure you want to  ${status === "gameon" ? " <span style='color:#2aa96b;'> Turn On </span> " : " <span style='color:#e60e38;'> Turn Off </span> "}  <span style="font-weight: 900;">${lotteryName}</span> ? `
        );
...
<a class='page-link' href='#' data-page='${i}'>${label}</a>
    ...
    $(document).on("click", ".addLottery", function (evt) {
        ...
        const stoptime = rawStopTime.length === 5 ? rawStopTime + ":00" : rawStopTime;
            ...
            function (response) {
                ...
                try {
                    ...
                    if (result.status === "success") {
                        ...
                        showToast("Already Exists", result.message || "Lottery already added", "warning");
                    } else {
                        showToast("Heads up!!", result.message || "Lottery Game failed", "info");
                    }
                } catch (e) {
...
$(document).on("click", ".update-game-image-btn", function () {
    const gameId = $("#lottery_game_id").val();
    ...
    formData.append("lottery_logo_file", selectedFile);

    $.ajax({
        url: `../game/Updateimage/${gameId}/${filename}`,
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
        success: function (res) {
            ...
            if (response.status === "success") {
                showToast("Success", response.message || "Image updated successfully", "success");
        ...
        error: function () {
            showToast("Error", "Upload failed due to a server issue.", "error");
        }
    });
});
...
//         selectedFile = file; // ✅ Store it persistently
...
//                 $("#lottery_logo_file").val("");
//                 $("#imagePreview").hide().attr("src", "#");
//                 selectedFile = null; // ✅ Clear after upload
//             } else {
//                 showToast("Error", response.message || "Upload failed", "error");
//             }
//         },
...
// $(document).on("click", ".update-game-image-btn", function () {
//     const gameId = $("#lottery_game_id").val();
//     const fileInput = document.getElementById("lottery_logo_file");
...
//     // console.log("Uploading image for Game ID:", gameId);
//     // console.log("Selected file:", filename);

//     const formData = new FormData();
//     formData.append("lottery_logo_file", file);
