$(() => {
    ...
    $("#toggle-lottery-msg").html(
      `Are you sure you want to  ${
        status === "gameon"
          ...
          : " <span style='color:#e60e38;'> Turn Off </span> "
      }  <span style="font-weight: 900;">${lotteryName}</span> ? `
    );
 ...
 <td><img src="${sanitizeHTML(data.lottery_image)}" alt="lottery icon" width="40"></td>
      ...
      <td id="td-sorting-weight-${data.id}">${
      data.sort_weight[data.id] ?? 0
    }</td>
      <td>${data.lottery_type}</td>
      <td>System</td>
      <td></td>
...
<a class='page-link' href='#' data-page='${i}'>${label}</a>
  ...
  function tableScrolluserListsquota() {
    const tableContainerUsersquota =
      document.querySelector(".table-wrapperbaic");
  ...
  $(document).on("click", ".addLottery", function (evt) {
    ...
    if (!name ||!alias ||!gamegroups ||!lotteryType ||!numberOfBalls ||!min_ball ||!max_ball ||!secondsperissue ||isNaN(secondsperissue) ||parseInt(secondsperissue) <= 0 ||!starttime ||!stoptime ||!lotterymodel ||!logoFileName ) {
      showToast(
        "Heads up!!","All fields are required and seconds per issue must be a positive number", "info" );
      return;
    }
  ...
  $(document).on("click", "#upload", function () {
    ...
    $("#lottery_logo_file").val("");
        ...
        showToast("Error", response.message || "Image upload failed.", "error");
    }
    },
  ...
  document.getElementById("lottery_logo_file")
