$(function() {
  ...
  async function fetchemaildata(page, pageLimit) {
    try {
      const response = await fetch(
        `../admin/fetchemaildata/${page}/${pageLimit}`
      );
      ...
      renderemailPagination(
        ...
        page,
        pageLimit,
        (newPage, pageLimit) => fetchemaildata(newPage, pageLimit)
      );
      document.getElementById("paging_infoemail").innerHTML =
  ...
  async function filteremail(
    ...
    try {
      let response = await fetch(
        `../admin/filteremail/${emailprovider}/${emailstatus}/${startemail}/${endemail}/${currentPage}/${pageLimit}`
      );
      const data = await response.json();
      //  console.log(data)
      ///return
  ...
  $(document).on("click", "#addemailsettings", function() {
    $.post(`../admin/savedemailpreferencestate`, function(response) {
      ...
      //  return
      let data =
        typeof response === "string" ? JSON.parse(response)[0] : response;
  ...
  $(document).on("click", ".saveemail", function() {
    ...
    //   $(".form-reset").val('');
    $.post(`../admin/emailaddprovider/${emailprovider}/${sendename}`, function(
      response
    ) {
