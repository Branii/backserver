$(() => {
  ...
  $(document).on("click", ".update-gamegroup", function() {
    const gameItems = $(
      $($(this).parents(".lbp-gamegroup-wrapper")[0]).find(
        ".lbp-gameitem-wrapper"
      )[0]
    ).find(".lbp-gameitem-parent");
    let data = [];
    for (let index = 0; index < gameItems.length; index++) {
      ...
      data.push({
        ...
        odds: odds,
        max_amt: maxAmt,
        max_tot_amt: maxTotAmt
      });
    }

    data = JSON.stringify(data);
    $.ajax({
      url: `../game/updateGameGroupData/${data}`,
      type: "GET",
      beforeSend: function() {
      ...
      success: function(response) {
        if (response === "Game id not found!!") {
          showToast(
            "Error",
            "Sorry the lottery selected is not yet ready.",
            "error"
          );
          return;
        }
        ...
        showToast(
          "Success",
          "Lottery Bonus Parameter Successfully updated.",
          "success"
        );
      },
  ...
  $(document).on("click", "#lbp_search", async function() {
    ...
    let lotteryText = $("#allGameNamesLottery option:selected")
      .text()
      .toLowerCase();
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (lotteryModel === "twosides") {
          ...
          lotteryText =
            lotteryTextCaps === "Mark6" || lotteryTextCaps === "Happy8"
              ? lotteryTextCaps
              : lotteryText;
          ...
          groupNames.forEach(groupName => {
            ...
            if (gameNames.length > 0) {
              gameNames.forEach(gameName => {
                markup += boardGamesMarkup(
                  response[groupName][gameName],
                  gameName,
                  lotteryText,
                  groupName
                );
          ...
          console.log(response);
          lotteryText =
            lotteryText === "5d" || lotteryText === "pk10"
              ? lotteryText
              : lotteryTextCaps;
      ...
      <input type="text" class="form-control lbp-gameitem-input" value="${element.total_max_bet_amount}" placeholder="Tot. Max. amt" id="lbp-max-tot-amt-${element.labelid}"></div></div></div>`;
            ...
            innerMarkup = broadBetParentMarkup(
              innerMarkup,
              "",
              gameID.join(","),
              state
  ...
  $(document).on("change", ".lpd-update-lottery", function() {
    ...
    const element = this;
    $.ajax({
      url: `../game/toggleTwosidesLotteryState/${gameID}`,
      type: "POST",
      beforeSend: function() {
        $($(element).find("i")[0])
          .removeClass("bx-check-double")
      ...
      success: function(response) {
        ...
        if (response.data == 0) {
          ...
          showToast(
            "Error",
            "Update unsuccessfull, please try again later.",
            "error"
          );
          return;
        }
  ...
  $(document).on("change", "#allmodels,#allGameNamesLottery", function() {
    ...
    if (lotteryModel === "twosides") {
      ...
      lotteryGameGroups.forEach((element, index) => {
        html += `<option value="${gameGroupsName[lotteryType][
          index
        ]}"> ${element}</option>`;
      });
      ...
      gameGroups.forEach((element, index) => {
        html += `<option value="${element}"> ${SanitizedFantanGameGroupsName[
          lotteryType
        ][index]}</option>`;
      });
      $("#game_groups").html(html);
...
const FantanGameGroupsName = {
  1: {
    and: ["And Value"],
    fantan1: ["Pass", "Positive", "Three Doors"],
    fantan2: [
      "fived_fantanHorn",
      "fived_fantanHorn1",
      "fived_fantanHorn2",
    ...
    main: ["Dragon Tiger Fight", "Sum of 5 digits", "Sweet Roses"],
    position: ["Dragon Tiger Fight", "Sum Of Five Digits"],
    sum_of: ["Dragon Tiger Fight", "Sum Of One Digits"]
  },
  2: {
    fantan1: ["fantan1Pass", "fantan1Position", "fantan1three"],
    fantan2: [
      "pk10_fantan2",
      "pk10_fantan2Horn",
      "pk10_fantan2Horn1",
    ...
    main: ["Mainbsoe", "Mainbsoe1", "mainnumber"],
  ...
  10: {
    ...
    main: ["happy8mainBSOE", "happy8mainColor", "happy8mainElement"]
  }
};
...
const boardGamesGameGroups = {
  "5d": {
    Baccarat: ["Baccarat"],
    Bull: ["Bull", "BullBSOE", "DTT", "Maximum", "Minimum"],
    Stud: [
      "Stud",
      "StudFirstThree",
      "StudFirstTwo",
  ...
  fast3: {
    combo: ["AnyTripples"],
    one: ["FBSOE", "Point"],
    two: ["AnyTripple", "AnyTwo", "GuessNumber", "OnePair"]
  },
  eleven5: {
    "Tab one": [
      "ElevenBSOE",
  ...
  Mark6: {
    "Tab One": ["ColorWave", "GuessNumbers", "MBSOE"],
    "Tab Two": ["ElementSeven"]
  },
  ...
  if (data.names === undefined && data.name === undefined) {
    ...
    data.data.forEach((element, index) => {
      if (element.key.trim() != "P/F") {
        key = element.gameid;
        if (
          (lottery == 8 && gameGroup == "Row No.") ||
          (lottery == 6 && gameGroup == "Pick") ||
          (lottery == 8 && gameGroup == "Specific No.")
        ) {
...
">
  ...
  margin-right: 0.8rem;
"><input type="checkbox" class="lpd-form-check-input lpd-update-lottery" id="lpd-gameid-${gameID}" datas="standard" role="switch" ${state ==
  "active"
    ? "checked"
    : ""} >
...
const twosidesUIItem = element => {
  ...
  const condition1 =
    ...
    ($("#allGameNamesLottery").val() == 6 ||
      ($("#allGameNamesLottery").val() == 5 &&
        $("#game_groups").val() == "2 No. Combo")) &&
    element.label < 10;

  const condition2 =
    isNumber(element.label) &&
    ($("#allGameNamesLottery").val() == 5 &&
      $("#game_groups").val() == "3 No. Combo") &&
    element.label < 10;
...
const boardGamesMarkup = (games = [], title = "", lottery = "", groupName) => {
  ...
  games.forEach(element => {
    ...
    if (lottery == "fast3" && title == "GuessNumber") {
      ...
      <span class="lbp-gameitem-name" style="width:6.5rem;">${lottery ===
      "eleven5"
