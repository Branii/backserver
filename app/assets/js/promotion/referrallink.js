$(function() {
  ...
  async function filterUserlinks(
    username,
    linkstart,
    linkenddate,
    currentPage,
    ...
    try {
      const response = await fetch(
        `../promotion/filterUserlinks/${username}/${linkstart}/${linkenddate}/${currentPage}/${pageLimit}`
      );
      const data = await response.json();
      ...
      renderfinacesPagination(
        ...
        pageLimit,
        (newPage, pageLimit) =>
          filterUserlinks(username, linkstart, linkenddate, newPage, pageLimit)
      );
      document.getElementById("paging_inforeferal").innerHTML =
  ...
  function renderfinacesPagination(
    totalPages,
    currentPage,
    pageLimit,
    callback
  ...
  function tableScrollLinks() {
    const tableContainerLinks = document.querySelector(
