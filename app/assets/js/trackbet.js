      ...
      htmls += `
                    <tr>
                        ...
                        <td> <span class="badge fw-semibold py-1 w-85 bg-success-subtle text-success">${
                          trackstatus[item.track_status]
                        }</span></td>
                        <td><i value='${item.track_token}_${
        item.game_type_id
      }' class='bx bx-info-circle trackinfo' style='color:#868c87;font-size:18px;cursor:pointer;'></i></td>
                    </tr>
                `;
    });
    return htmls;
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (value == test) {
        htmlbet += `
                  <td>${value}</td>
                   <td class="${
                     key === "user_selection" ? "bet_userSelection" : ""
                   }">
                    <textarea class="form-control"   readonly style="height: 75px;">${
                      obj[key]
                    }</textarea>
                    </td>`;
      } else {
  ...
  async function filterTrack(
    ...
    try {
      const response = await fetch(
        `../admin/filterTrackdata/${usernames}/${trackstatus}/${trackcode}/${tracklotery}/${startdatetrack}/${enddatetrack}/${currentPagetrack}/${pageLimit}`
      );
