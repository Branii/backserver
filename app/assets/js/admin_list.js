...
<td>
    ...
    async function getAdminLogs(currentPage, pageLimit, adminId) {
        try {
            const response = await fetch(`../admin/adminlogs/${currentPage}/${pageLimit}/${adminId}`);
            const data = await response.json();
    ...
    async function filterAdminLogs(currentPage, pageLimit, adminId, datefrom, dateto) {
        try {
            const response = await fetch(`../admin/filterAdminLogs/${currentPage}/${pageLimit}/${adminId}/${datefrom}/${dateto}`);
            const data = await response.json();
            console.log(data.adminLogs);
            renderAdminLogs(data.adminLogs);
    ...
    async function searchlogs(currentPage, pageLimit, adminId, query) {
        try {
            const response = await fetch(`../admin/searchlogs/${currentPage}/${pageLimit}/${adminId}/${query}`);
            const data = await response.json();
            console.log(data.adminLogs);
            renderAdminLogs(data.adminLogs);
    ...
    $(document).on("click", ".admin_pro", function () {
        ...
        $("#adminprofile").attr("src", "http://localhost/admin/app/assets/images/profile/" + adminObject.profile_picture);
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", function (e) {
            if (!$(e.target).closest("#transuseradmin, #adminuserAccountDropdown").length) {
                $("#adminuserAccountDropdown").hide(); // ✅ fixed
            }
        });
    ...
    function fetchbetUser(query) {
        ...
        $.post(`../admin/Searchusernamesss/${encodeURIComponent(query)}`, function (response) {
            try {
                ...
                response.forEach((admin) => {
                    ...
                    if (fullName.toLowerCase().includes(query.toLowerCase())) {
                        ...
                        displayText = fullName || username || email || contact || "Unknown";
