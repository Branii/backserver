...
>>>>>>> d33f9770d94f1c2db3005cde4336a0003a8f4350:app/assets/js/payment/payment_platforms.js
  ...
  async function fetchPaymentPlatform(page, pageLimit) {
    try {
      const response = await fetch(
        `../payment/fetchPaymentPlatform/${page}/${pageLimit}`
      );
  ...
  async function filterpayment(
    ...
    try {
      let response = await fetch(
        `../payment/filterpayments/${partnerID}/${curencytypes}/${stautspayment}/${startdepay}/${enddepay}/${currentPage}/${pageLimit}`
      );
      const data = await response.json();
  ...
  $(document).on("click", ".savemoneybtn", function() {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (!value || value.trim() === "") {
        ...
        const requiredMessage = replacePlaceholder(
          fieldRequiredTemplate,
          translatedFieldName
        );
        showToast(headsUpText, requiredMessage, "info");
  ...
  $(document).on("click", ".updatepaybtn", function() {
    ...
    $(".loaderpayanup")
      ...
      .addClass("bx-loader-circle bx-spin loader");
    $.post(
      `../payment/updateplatform/${typecurrency}/${maxiamounts}/${minamount}/${statecurrent}/${paymentids}`,
      function(response) {
        $(".loaderpayanup")
          .removeClass("bx-loader-circle bx-spin loader")
          .addClass("bx-send");
