$(function() {
    ...
    data.forEach(item => {
      ...
      htmls += `
                    <tr>
                        ...
                        <td>${formatMoney(item.done_amount) +
                          "/" +
                          formatMoney(item.total_amount)}</td>                  
                        <td>${formatMoney(item.win_amount)}</td>
                        <td>${trackrule}</td>
                        <td>${item.server_date + " / " + item.server_time}</td>
                        <td>${timezone}</td>
                        <td> <span class="badge fw-semibold py-1 w-85 bg-success-subtle text-success">${trackstatus[
                          item.track_status
                        ]}</span></td>
  ...
  const Showtrackbet = (data, obj) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (value == test) {
        htmlbet += `
                  <td>${value}</td>
                   <td class="${key === "user_selection"
                     ? "bet_userSelection"
                     : ""}">
                    <textarea class="form-control"   readonly style="height: 75px;">${obj[
                      key
                    ]}</textarea>
                    </td>`;
      } else {
  ...
  async function fetchtrackdata(page, pageLimit) {
    try {
      const response = await fetch(
        `../businessflow/trackdata/${page}/${pageLimit}`
      );
      ...
      // Render pagination
      renderPaginationtrack(
        data.totalPages,
        page,
        pageLimit,
        (newPage, pageLimit) => fetchtrackdata(newPage, pageLimit)
      );
      document.getElementById("paging_infotrack").innerHTML = `${translator[
        "Page"
      ]} ${page} ${translator["Of"]} ${data.totalPages} ${translator["Pages"]}`;
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }

  async function filterTrack(
    usernames,
    trackstatus,
    trackcode,
    tracklotery,
    ...
    try {
      const response = await fetch(
        `../businessflow/filterTrackdata/${usernames}/${trackstatus}/${trackcode}/${tracklotery}/${startdatetrack}/${enddatetrack}/${currentPagetrack}/${pageLimit}`
      );
  ...
  function renderPaginationtrack(
    totalPages,
    currentPagetrack,
    pageLimit,
    callback
  ...
  async function fetchLotteryname() {
    try {
      const response = await fetch(
        `../businessflow/fetchLotteryname/${partnerID}`
  ...
  async function fetchAllToken(tracktoken) {
    try {
      const response = await fetch(
        `../businessflow/getAllTokenbet/${tracktoken}`
      );
      const data = await response.json();
