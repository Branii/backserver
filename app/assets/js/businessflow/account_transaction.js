$(function () {
  ...
  const AccountTransactions = (data) => {
    ...
    const statusColor = {
      1: { title: translator["Deposit"], color: "#4CAF50" },
      2: { title: translator["Win Bonus"], color: "#FF9800" },
      3: { title: translator["Bet Awarded"], color: "#03A9F4" },
      4: { title: translator["Withdrawal"], color: "#F44336" },
      5: { title: translator["Bet Deduct"], color: "#E91E63" },
      6: { title: translator["Bet Cancelled"], color: "#9E9E9E" },
      7: { title: translator["Rebates"], color: "#8BC34A" },
      8: { title: translator["Self Rebate"], color: "#00BCD4" },
      9: { title: translator["Sending Red Envelope"], color: "#FF5722" },
      10: { title: translator["Red Envelope Receive"], color: "#795548" },
      11: { title: translator["Bet Refund"], color: "#FFC107" },
      13: { title: translator["Profit Limit"], color: "#ff06f3ff" }
    };

    const completes = translator["Completed"];
    const formatTimestamp = (t) =>
      t && typeof t === "string" && t.trim()
        ? t.trim().replace(" ", " / ")
        : "N/A";

    const filtered = data.filter((item) => item.order_type !== 12);
    ...
    filtered.forEach((item) => {
      ...
      html += `
            <tr class="trow">
                ...
                <td>${username}</td>
                <td>${item.name}</td>
                <td><i class='bx bxs-circle' style='color:${
                  statusColor[item.order_type]?.color || "#000"
                };font-size:8px;margin-right:5px;'></i>${
        statusColor[item.order_type]?.title || unknownText
      }</td>
                <td>${
                  formatMoney(item.account_change) < 0
                    ? formatMoney(item.account_change)
                    : `+ ${formatMoney(item.account_change)}`
                }</td>
                ...
                <td><i value='${item.order_id}_${item.game_type}_${
        item.order_type
      }' class='bx bx-info-circle tinfo' style='color:#868c87;font-size:18px;cursor:pointer;'data-bs-toggle="modal" data-bs-target="#signup-modal"></i></td>
            </tr>`;
    });
  ...
  const FromTable = (transactiondata, obj) => {
    ...
    Object.entries(transactiondata).forEach(([key, value]) => {
      let test =
        value !== translator["Bet Selection"] && langMap[value]
          ? langMap[value]
          : translator["Bet Selection"];
      if (value == test) {
        html += `
                   <td>${value}</td>
                  <td class="">
  ...
  async function fetchTrasaction(page, pageLimit) {
    try {
      const response = await fetch(
        `../businessflow/transactiondata/${page}/${pageLimit}`
      );
      const data = await response.json();
      $("#mask").LoadingOverlay("hide");
      render(data.transaction);
      renderPagination(data.totalPages, page, pageLimit, (newPage, pageLimit) =>
        fetchTrasaction(newPage, pageLimit)
      );
      document.getElementById(
        "paging_info"
      ).innerHTML = `${translator["Page"]} ${page} ${translator["Of"]} ${data.totalPages} ${translator["Pages"]}`;
    } catch (error) {
  ...
  async function filterTrasaction(
    transusername,
    transactionId,
    ordertypetrans,
    partneruid,
    ...
    try {
      const response = await fetch(
        `../businessflow/filtertransactions/${transusername}/${transactionId}/${ordertypetrans}/${partneruid}/${startdatrans}/${enddatetrans}/${currentPage}/${pageLimit}`
      );
      ...
      document.getElementById(
        "paging_info"
      ).innerHTML = `${translator["Page"]} ${currentPage} ${translator["Of"]} ${data.totalPages} ${translator["Pages"]}`;
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }
  ...
  function processTransactionData(transactiondata) {
    ...
    if (transactiondata.deposit.deposit_withdrawal_type == 1) {
      total_income =
        '<span style="color:;">+' +
        transactiondata.deposit.deposit_and_withdrawal_amount +
        "</span>";
      deposit_withdrawal_types = `<span style="color:;">${translator["Deposit"]}</span>`;
      ...
      deposit_withdrawal_types = `<span style="color:;">${translator["Withdrawal"]}</span>`;
    }
    ...
    return {
      ...
      recharge_balance: formatMoney(
        transactiondata.deposit.recharge_balance_in_advance
      ),
      deposit_time:
        transactiondata.deposit.date_created +
        " / " +
        transactiondata.deposit.deposit_and_withdrawal_time,
  ...
  function populatedepositeTable(transactiondata) {
    ...
    row.insertCell(6).textContent = translator[processedData.remark];
  }
  ...
  async function fetchTrasactionBet(transactionId) {
    try {
      const response = await fetch(
        `../businessflow/getTransactionBet/${transactionId}`
      );
      ...
      if (transactiondata.deposit) {
        ...
        // Show normal transaction layout
  ...
  function getTranslation(key) {
    const span = document.getElementById(
      `trans-${key.toLowerCase().replace(/\s+/g, "-")}`
    );
