$(function () {
    ...
    const AccountTransactions = (data) => {
        ...
        filtered.forEach((item) => {
            ...
            <tr class="trow">
                ...
                <td><i value='${item.order_id}_${item.game_type}_${item.order_type}' class='bx bx-info-circle tinfo' style='color:#868c87;font-size:18px;cursor:pointer;'data-bs-toggle="modal" data-bs-target="#signup-modal"></i></td>
            </tr>`;
        });
   ...
   async function fetchTrasactionBet(transactionId) {
    try {
        ...
        console.error("Error fetching data:", error);
        $("#row1").html(`<div class="text-danger text-center">Failed to load data</div>`);
        $("#row2").empty();
    }
   }
    ...
    $(document).on("click", ".tinfo", function () {
        ...
        // $("#row1").empty();
          $("#row1").html('<div class="text-center my-3"><div class="spinner-border text-primary" role="status"></div></div>');
        $("#row2").empty();
