...
// showToast(headsUpText, failedText, "info");
  ...
  const inactiveText =
    document.getElementById("translate-inactive").dataset.value;
                         ...
                         <td>${bankstatus}</td>
                          <td>
                           <div class="dropdown">
                            ...
                            <a class="dropdown-item kanban-item-edit cursor-pointer d-flex align-items-center gap-1 editpayment" href="javascript:void(0);" datas ="${
                              item.bankid
                            }"> 
                                <i class="bx bx-edit fs-5" ></i>${EdittText}
                            </a>
                                <a class="dropdown-item deletepayment cursor-pointer d-flex align-items-center gap-1" href="javascript:void(0);" datas="${
                                  item.bankid
                                }">
  ...
  async function fetchPaymentPlatform(page, pageLimit) {
    try {
      const response = await fetch(
        `../admin/fetchPaymentPlatform/${page}/${pageLimit}`
      );
  ...
  function renderpaymentPagination(
    ...
    document
      .querySelectorAll("#paginationpayments .page-link")
      .forEach((link) => {
  ...
  async function filterpayment(
    ...
    try {
      let response = await fetch(
        `../admin/filterpayments/${partnerID}/${curencytypes}/${stautspayment}/${startdepay}/${enddepay}/${currentPage}/${pageLimit}`
      );
      const data = await response.json();
 ...
 const translatedFieldName = fieldNameTranslations[key] || key;
    const requiredMessage = replacePlaceholder(fieldRequiredTemplate, translatedFieldName);
    showToast(headsUpText, requiredMessage, "info");
  ...
  $(document).on("click", ".updatepaybtn", function () {
    ...
    $(".loaderpayanup")
      ...
      .addClass("bx-loader-circle bx-spin loader");
    $.post(
      `../admin/updateplatform/${typecurrency}/${maxiamounts}/${minamount}/${statecurrent}/${paymentids}`,
      function (response) {
        $(".loaderpayanup")
          .removeClass("bx-loader-circle bx-spin loader")
          .addClass("bx-send");
  ...
  function tableScrollpayment() {
    const tableContainerpayment = document.querySelector(
      ".table-wrapperpayment"
    );
